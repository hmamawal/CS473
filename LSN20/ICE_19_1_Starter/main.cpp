...
int main()
{
    ...
    BasicShape die = importer.loadFiles("models/texture_die",import_vao);
    ...
    //BasicShape ship = importer.loadFiles("models/ship",import_vao);
    Avatar ship(importer.loadFiles("models/ship",import_vao),
            IMPORTED_BASIC,90.0f,glm::vec3(2.0,1.0,0.0));
    // trouble shooting
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // -----
        ProcessInput(window);
        ship.ProcessInput(window,delta_time); // *** check if time passed in is correct

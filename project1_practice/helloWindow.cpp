...
// Render duck as a simple block (unchanged)
void renderDuck(const Duck& duck, unsigned int shaderProgram, unsigned int VAO) {
    float vertices[] = {
        duck.x, duck.y,                     0.0f, 0.0f,
        duck.x + ENTITY_SIZE, duck.y,       1.0f, 0.0f,
        duck.x + ENTITY_SIZE, duck.y + ENTITY_SIZE, 1.0f, 1.0f,
        duck.x, duck.y,                     0.0f, 0.0f,
        duck.x + ENTITY_SIZE, duck.y + ENTITY_SIZE, 1.0f, 1.0f,
        duck.x, duck.y + ENTITY_SIZE,       0.0f, 1.0f
    };
    glBindBuffer(GL_ARRAY_BUFFER, VAO);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertices), vertices);
    glUseProgram(shaderProgram);
...
// Render fox as a simple block (unchanged)
void renderFox(const Fox& fox, unsigned int shaderProgram, unsigned int VAO) {
    float vertices[] = {
        fox.x, fox.y,                     0.0f, 0.0f,
        fox.x + ENTITY_SIZE, fox.y,       1.0f, 0.0f,
        fox.x + ENTITY_SIZE, fox.y + ENTITY_SIZE, 1.0f, 1.0f,
        fox.x, fox.y,                     0.0f, 0.0f,
        fox.x + ENTITY_SIZE, fox.y + ENTITY_SIZE, 1.0f, 1.0f,
        fox.x, fox.y + ENTITY_SIZE,       0.0f, 1.0f
    };
    glBindBuffer(GL_ARRAY_BUFFER, VAO);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertices), vertices);
    glUseProgram(shaderProgram);

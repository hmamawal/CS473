...
int main()
{
    ...
    Avatar avatar(importer.loadFiles("models/ship",import_vao),90.0f,
                  glm::vec3 (4.0,1.0,-1.0));
    avatar.SetScale(glm::vec3(0.5,0.5,0.5));
    ...
    shader_program.setMat4("model",identity_matrix);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // -----
        ProcessInput(window);
        avatar.ProcessInput(window,delta_time);
        ...
        //draw the floor
        glm::mat4 floor_transform(1.0);
        floor_transform = glm::translate(floor_transform,glm::vec3(0.0,0.0,-0.01));
        floor_transform = glm::rotate(floor_transform,glm::radians(-90.0f),
                                      glm::vec3(1.0,0.0,0.0));
        shader_program.setBool("is_textured",true);
        shader_program.setBool("is_imported",false);
        shader_program.setMat4("model",identity_matrix);
        shader_program.setMat4("transform",floor_transform);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D,floor_texture);
        floor.Draw();

        avatar.Draw(&shader_program);


        //keep the user off the ground.
...
void ProcessInput(GLFWwindow *window)
{
    ...
    if (glfwGetKey(window,GLFW_KEY_RIGHT)==GLFW_PRESS) {
        angle_y -= rotate_speed * delta_time;
    }
